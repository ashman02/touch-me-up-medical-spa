...
const Services = () => {
	return (
		<main className="flex flex-col gap-32 overflow-hidden">
			<section className="my-container h-screen min-h-[450px] w-full flex-gap items-center justify-center">
				<h1 className="hero-heading text-center">
					<div>{servicesData.hero.heading1}</div>
					<div>{servicesData.hero.heading2}</div>
				</h1>
				<p className="paragraph-text text-gray-500 text-center md:w-10/12 lg:w-3/4">
			...
			<section className="flex-gap">
				...
				<div className="caraousel overflow-hidden pl-2 md:pl-3 lg:pl-4">
					<div className="flex gap-2 md:gap-3 lg:gap-4 overflow-visible">
						{servicesData.popularServices.servicesArray.map(
							(service) => (
								<div
									key={service.id}
									className="w-[320px] md:w-[480px] h-[600px] lg:h-[calc(100vh-80px)] lg:min-h-[400px] md:h-[650px] shrink-0 overflow-hidden"
								>
									<ServiceCard
										title={service.name}
										img={service.img}
			...
			<section className="my-container cta-section h-screen min-h-[450px] w-full flex-gap items-center justify-center bg-gray-100">
				<SectionHeading
